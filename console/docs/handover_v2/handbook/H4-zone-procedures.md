...
USB firmware upgrades never go backwards either. A board with newer firmware than this computer's build (for example flashed from newer code on another computer) is listed as **by hand** and left alone. So is a board whose version the console cannot compare with the build. (Simulation-verified)
<kr>USB 펌웨어 업그레이드도 이전 버전으로 돌아가지 않습니다. 이 컴퓨터의 빌드보다 새 펌웨어를 가진 보드(예: 다른 컴퓨터에서 더 새 코드로 플래시한 보드)는 **by hand**로 표시만 하고 건드리지 않습니다. 콘솔이 빌드와 버전을 비교할 수 없는 보드도 마찬가지입니다. (Simulation-verified)</kr>
...
- Downgrading firmware: a board newer than this computer's build, or with a version it cannot compare, is listed as **by hand**. <kr>펌웨어 다운그레이드: 이 컴퓨터의 빌드보다 새 보드나 버전을 비교할 수 없는 보드는 **by hand**로 표시만 합니다.</kr>
