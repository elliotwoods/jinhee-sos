...
- Workstation **On the reader** › **On each tag** back on **Flash 2 s** (a zone action left selected sets the zone of every cube tapped on the reader).
...
- Behaviour: runs in the board's hub session (opened automatically while Behaviour › Open a live session… is on), whether or not the page is open; the session reads the UID back with `nfc_poll` when idle. Each tag placed resolves its owner (committed `uid` first, else `pending_uid`) and does one of:
  ...
  - `zone:N`: one `set_zone` to that cube (same as the **SET_ZONE** buttons); the zone stays.
