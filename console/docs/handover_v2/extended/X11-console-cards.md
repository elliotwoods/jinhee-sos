...
| Behaviour › Workstation reader: signal the cube whose tag is read (a 2 s flash, or the zone chosen on the Workstation page), whether or not the page is open; never while the radio is busy or a main show is running | `reader_flash` | on | One setting for every Workstation (firmware with the `nfc` role; never the legacy pairing station). Same setting as the **Signal the cube when its tag is read** switch on the Workstation **On the reader** card. The action (`flash` \| `zone:0`…`zone:4`, default `flash`) is saved separately in metadata `console_reader_action`. See "Workstation panel › On the reader" below |
...
- Over-the-air lines: **Zone databases** "v{n}: X current · Y behind · Z updating" (plus the walking relay's message while it publishes, and "Over-the-air zone updates are off (Settings › Automatic updates)" when `auto_zone_db_radio` is off and zones are behind); **Main show** "v{n}: X current · Y behind" (plus the registry message while publishing); "{n} more behind, out of range: updated when a radio next hears them"; "{n} heard over the air in the last day with older firmware: plug in over USB to upgrade" (expandable list: label · current → build; only zones and cubes that are older than the build or cannot be ordered against it). "Behind" counts only boards in range now.
...
USB upgrades (`auto_firmware_usb`) only upgrade, never downgrade (`autoupgrade.compare_versions`, working-tree change of 24 Sept, uncommitted when written). A board is flashed only when its reported version is **older** than this computer's build. Versions compare numerically within one family prefix (`desert-2.3.0` < `desert-2.4.0`, `pool-3.10.0` > `pool-3.9.0`); for cubes the suffix counts too (`v1.7.0-USB.2` > `v1.7.0-USB.1`, a plain `v1.7.0` is older than `v1.7.0-USB.1`). A board that is **newer** is listed with state `report` (pill **by hand**) and the reason "Newer than this computer's build (‹version›); not downgraded". A board whose version **cannot be ordered** (unparseable, a different family prefix, or a different cube suffix label such as `-RC.1` against `-USB.1`) is listed the same way with "Cannot tell whether ‹running› is older than ‹version›; not flashed automatically (flash it by hand if it should change)". The one deliberate family crossing stays: a legacy pairing station / General Radio becomes a Workstation.

| Board | Condition | What runs | Notes |
|---|---|---|---|
| Zone plate (not a pool radio) | Its `?` firmware is older than the sketch's `FIRMWARE_VERSION` | Zone flasher pipeline, keeping profile, point, name, params and RX gain from its own report | Needs a configured board ("Unconfigured board: give it an identity on the Flash page once") and a published zone database ("No zone database is published yet") ({{page:X09}}) |
| Pairing station (`nct-pairing-*`) or General Radio (`general-radio-*`) | Always | Becomes workstation-1.0.0 | An older `workstation-*` is upgraded too; a newer or unorderable one is only listed. The protected station 3C:0F:02:AD:83:24 is never planned |
| Mainshow controller | Older than mainshow-1.3.0 | Mainshow controller firmware | Held while the show runs; a newer one is only listed |
| Cube | Verified `?` FW older than `core.VERSION` (v1.7.0-USB.1) | Flash-page pipeline: firmware plus the published show | Only while Register and the Flash page are off; excluded MACs skipped ({{page:X03}}) |
| Pool radio | Older than the build | Listed only (**by hand**) | "Pool radios and the pool central are a matched set: upgrade them together by hand"; newer or unorderable: listed with the not-downgraded / cannot-tell reason |
| Pool central, preshow bridge | Older than the sketch | Listed only | "No automatic flash for this board; upload it from its sketch by hand" (pool central adds the matched-set note); newer or unorderable: listed with the not-downgraded / cannot-tell reason |
...
- Workstation **On the reader** › **Signal the cube when its tag is read** back on **Flash 2 s** (a zone action left selected sets the zone of every cube tapped on the reader).
...
- Hardware buttons run on one click with a warning tooltip. Destructive ones (**Force flash**, **Force write**) need **Hold to confirm**: the button carries a padlock, a fill sweeps across it during the 0.7 s hold and the padlock stays open while the command runs (`console/web/components/actions.js::HoldButton`; from `63d82dc` the visible label no longer starts with "Hold:", which stays only as the accessible name).
...
### Workstation panel › On the reader: tag-read action

- Controls (top of the **On the reader** card; shown only when the board reports the `nfc` role, i.e. Workstation firmware; hidden on a legacy pairing station): one selector **Signal the cube when its tag is read** (radio group labelled **On each tag**) with the choices **Off** · **Flash 2 s** · **idle** · **preshow** · **desert** · **pool** · **mainshow** (zone tooltip "Sends SET_ZONE {zone} ({name}) to the cube."). The chosen one is highlighted; it is a setting, not a command. **Off** calls `radio.reader_flash {on: false}` (the action is kept), every other choice `{on: true, action}`; all call `radio.reader_flash {on?, action?}` (hardware kind: one click, warning tooltip; `device` is accepted and ignored). An unknown action is refused: "Unknown tag-read action ‹x›; use one of flash, zone:0, zone:1, zone:2, zone:3, zone:4".
- The rest of the card: the tag readout, **Stop** and **Open cube page**, and the tag history. It has no one-shot **Flash 2 s** or SET_ZONE buttons (removed 25 Sept): what happens to the cube on the reader is only the selector above. Manual flashes and zones: the Workstation **Pairing** tab (**Flash 2 s**) and **Cubes & show** (**Set … → zone**).
- Settings: `reader_flash` (default **on**, `console_settings`, one setting for every Workstation; also Settings › Behaviour) and metadata `console_reader_action` (`flash` default, or `zone:0`…`zone:4`). Both persist across restarts.
- Detail texts: flash: "Each tag placed on the reader makes its cube flash blue/red for 2 s, then idle white: the cube is registered and reachable. Anything you start takes over at once; nothing is sent while the radio is busy or a main show is running." · zone: "Each tag placed on the reader sets its cube to {zone} (SET_ZONE {n}). Nothing is sent while the radio is busy or a main show is running." · off: "Off: a tag on the reader is only shown here."
- Action tooltip (`uitext.py`): "On by default: each tag placed on a Workstation's reader makes the cube that owns it flash blue/red for 2 s (or, if chosen, sets its zone), whether or not the page is open, so you can see the tag, the inventory and the radio all agree." Hazard: "Visible on the cube: the flash ends in idle white, a zone stays set. Nothing is sent while the radio is busy or a main show is running; anything you start takes over at once." Needs: "A Workstation with its NFC reader working (not the legacy pairing station)."
- Behaviour: runs in the board's hub session (opened automatically while Behaviour › Open a live session… is on), whether or not the page is open; the session reads the UID back with `nfc_poll` when idle. Each tag placed resolves its owner (committed `uid` first, else `pending_uid`) and does one of:
  - `flash`: `Controller.background_flash(row, 2000)`, a 2 s identify (Controller mode `reader_flash`, phase `flashing`, message "Reader check: flashing cube #‹n› · ‹mac›", `#?` without a number; the status bar **Station** item shows the mode `reader_flash` meanwhile). It leaves the registration feedback and batch alone. `flash_done` ends it ("Reader check flash complete. Radio results do not verify visible LEDs."); an `error` ends it ("Reader check flash refused: ‹detail›"); no `flash_done` within 10 s stops it without dropping the link ("Reader check flash: no flash_done from the station; stopped it."). `yield_background()` ends it at once (sends `stop` first) when an operator starts anything: `available()`/`repair()`/`rename()` (registration, pairing, identify, send saved mapping), a unicast `radio.set_zone` colour, `zones.update`/`zones.update_all`, `inventory.set_role`. Cube-card buttons, **Pair new cubes (auto)** and the Register flow treat mode `reader_flash` as idle.
  - `zone:N`: one `set_zone` to that cube (same as **Cubes & show** › **Set … → zone**); the zone stays.
- Skipped (nothing sent) when: no inventory row owns the tag; the device is excluded; this Controller is busy or not connected; another Workstation link's Controller (not a `reader_flash`) is working with that MAC; a main show is known to run (`showedit.controller_show_running()` or the board's `status.show_running`: SET_ZONE, and so an identify, ends the show on the cube). A tag laid down while the radio is busy is read back with `nfc_poll` once it is free and acted on then; a tag the firmware reports with a `tag` event during a registration is left alone (result busy, no log).
- One action per placement: a UID acted on is not acted on again until it is seen leaving the reader outside its own flash (the identify's synthetic `tag_state` gaps are ignored); a re-handshake does not re-arm it.
- A USB cube identified during a tag-read flash does not stop it and does not log "Stopped the active pairing operation before pinning …" (`hub.identified()` skips mode `reader_flash`).
- No cube firmware change (identify and SET_ZONE are answered by every cube since v1.3); nothing written to cube NVS; no inventory write; not an `nfc_seen` scan.
- "Last automatic action" row (while the action is effective and something happened): `#<n> · <result> · <time>` (the UID when the device has no number). Results: **flashed** · **flashed (pending tag)** · **zone N sent** · **nothing sent: unknown tag** · **nothing sent: the radio was busy** · **nothing sent: excluded device** · **nothing sent: a main show is running**.
- Log lines (source `reader`): "Flashing cube #44 for 2 s: its tag ‹uid› is on the reader" (+ " (pending tag, not yet acknowledged)") · "Setting cube #44 to mainshow (SET_ZONE 4): its tag ‹uid› is on the reader" · "Tag-read action skipped: ‹mac› is an excluded device" · "Tag-read action skipped: another radio is working with cube #44" · "Tag-read action skipped: a main show is running and SET_ZONE would stop it on cube #44" · "Tag-read action not sent: ‹error›" · "Tag-read action: flash 2 s" / "Tag-read action: set zone 4 (mainshow)" · "Tag-read action turned ON for every Workstation" / "… OFF …". An unknown tag or a busy Controller logs nothing beyond the placement line ("Tag ‹uid› on the ‹label› reader: a tag the inventory does not know").
- Evidence: **Simulation-verified** (`console/tests/test_workstation.py::test_tag_read_flash_is_on_by_default_and_ends_by_itself`, `test_tag_read_flash_gives_way_to_everything`, `test_tag_read_can_set_a_zone_instead`, `test_tag_read_skips_a_cube_in_use_elsewhere_or_in_a_show`; `pairing_station/tests/test_station.py::test_background_flash_gives_way_to_every_operation`; a simulated console run). Commits `e5896f6` (behaviour) and `926ac6f` (one selector, on-demand buttons removed from the card, 25 Sept). Not yet run on a real Workstation ({{page:X13}}). Procedure: {{page:H3}} B3.
...
- Unidentified board panel › **Override inventory protection…** opens **Override inventory protection?** (KR 인벤토리 보호를 무시할까요?): refusal text, **Firmware** (Workstation / Mainshow controller), hold **Force write** (KR 강제 쓰기), **Cancel**. The dialog also opens by itself when a **Make this board a…** Workstation or Mainshow controller hold is refused by the inventory. Command `dongle.flash_force` (destructive); action label "Force write Workstation / controller firmware"; hazard "Overwrites whatever the board was: a zone leaves the show, a cube loses its LED firmware. A full backup is taken the first time. Hold to confirm." Disabled for a port that is not a flash candidate (the protected station) or while a job runs. Code-checked only ({{page:X02}}, {{page:X09}}).
...
- **‹job›: refused** → check the board's role. Force flash / **Force write** is never a fix; only for deliberately repurposing that exact board ({{page:X09}}).
...
- Only-upgrade rule (`compare_versions`, `not_older`): **Simulation-verified** (`console/tests/test_autoupgrade.py`: `test_versions_are_ordered`, `test_newer_boards_are_listed_never_downgraded`, `test_newer_workstation_and_unordered_plate_are_left_alone`; console suite 298 OK), uncommitted in the working tree on 24 Sept.
- With both switches on by default, plugging an out-of-date (older) board into the console computer now changes its firmware without a click: an old cube (v1.4.1-USB.2) once Register and Flash are off, the Mainshow controller #134 (mainshow-1.2.0 → 1.3.0), a General Radio (→ Workstation). Switch `auto_firmware_usb` off, or **Skip**, before plugging in a board that must stay as it is. **Code-checked**.
...
- Tag-read action: `console/sessions/workstation.py` (`reader_flash`, `reader_action`, `_reader_action`, `_held_elsewhere`, `_show_running`), `pairing_station/controller.py` (`background_flash`, `yield_background`), `console/commands.py` (`radio.reader_flash`, `settings`), `console/hub.py` (`reader_action`, `identified`), `console/uitext.py`, `console/web/panels/WorkstationPanel.js` (`ReaderCube`), `console/web/panels/sections.js` (Behaviour), `console/tests/test_workstation.py::test_tag_read_*`, `pairing_station/tests/test_station.py::test_background_flash_gives_way_to_every_operation`, commit `2739586` plus the uncommitted enable-default-tag-flash change
