...
- Setup then: creates `pairing_station/.venv`; installs `flashing_station/requirements.txt` (pinned pyserial 3.5, esptool 5.3.1) and `console/requirements.txt` (`pywebview==6.2.1`; `pythonnet==3.1.0` on Windows); validates the bundled cube manifest/binary hashes ("Firmware ready: …"); runs `scripts/sync_inventory.py` (shared Git inventory). Needs internet.
- The web inventory also needs `pairing_station/data/web_password`, which is not in Git: enter the shared password once per computer when the console asks.
...
3. Record which step failed (pip install or `scripts/sync_inventory.py`).
