...
- Releases (GitHub, tag `console-v<version>`, version = `CONSOLE_VERSION` in `console/state.py`): **console-v0.1.0** (25 Sept, from `88337cb`) and **console-v0.1.1** (25 Sept, from `74b8d24`: everything above plus the standalone-app documentation). Each carries the Mac disk image, the Windows zip, the shipped-tree zip and, from 0.1.1, the handover PDF.
