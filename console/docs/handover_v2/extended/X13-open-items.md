...
| **Release package:** agree which release to deploy (standalone app releases `console-v0.1.0` and `console-v0.1.1` are on GitHub with their firmware built in; see {{page:X10}}); the Windows app is unsigned and neither app has been used with boards yet; reconcile the PoolCentral 40 MHz flash recipe with the aggregate build ({{page:X02}}, {{page:X06}}); record the installed firmware and zone database version per board | Open | — | Code-checked |
...
| Workstation tag-read action (on by default) | Next bench session, on a real Workstation: place a registered cube's tag on the reader → the cube blinks blue/red for 2 s, then idle white, and "Last automatic action" says **flashed**; lift and place again → a second blink; choose **mainshow** in **Signal the cube when its tag is read** → the cube goes mainshow-ready; choose **Off** → nothing happens on the next tag; set it back to **Flash 2 s** | Not run | Simulation-verified (`console/tests/test_workstation.py::test_tag_read_*`, 4 tests) |
| Registration with the tag-read action on | Register a cube on the **Register** page with the switch on: the scan and registration proceed normally (the 2 s flash gives way, no "Stopped the active pairing operation" line), and the tag-read flash does not clear the registration banner | Not run | Simulation-verified (`pairing_station/tests/test_station.py::test_background_flash_gives_way_to_every_operation`) |
