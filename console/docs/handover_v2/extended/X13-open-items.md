...
| Workstation tag-read action (on by default) | Next bench session, on a real Workstation: place a registered cube's tag on the reader → the cube blinks blue/red for 2 s, then idle white, and "Last automatic action" says **flashed**; lift and place again → a second blink; choose **mainshow** under **On each tag** → the cube goes mainshow-ready; set it back to **Flash 2 s** | Not run | Simulation-verified (`console/tests/test_workstation.py::test_tag_read_*`, 4 tests) |
