...
| Sync switches local automatic numbering off | Every web sync (`pairing_station/web_sync.py`, "Independent computers cannot safely allocate the next free number") and `inventory_sync.apply`, which web sync uses, write `auto_number=0`; nothing sets it back to 1. The live database holds `0` |
| Numbers from the web (commit 6b26cdf) | With a stored web password (`hub.web_numbering()`), the console asks the web for new numbers: every 2 s for brand-new unnumbered, non-excluded cubes, and in the Register page's Number step. `POST /api/inventory/claim` returns the lowest free number ≥ 33 not held or claimed by another MAC and not in `exclude` (local numbers + 2, 22, 39, 43). Two computers can no longer pick the same number. Failure: the cube stays **Needs number**, retry every 60 s. Detail: {{page:X08}} › Cube numbers from the web. Code-checked; deployment of the web endpoint **To confirm** |
| Effect of `auto_number=0` | `Database.reserve()` gives a newly seen MAC (USB identification in `hub.identified`, radio discovery) **no number**: status `needs_number`, pill **Needs number**, detail "Choose Rename device to assign its label number", card **‹MAC› has no number** with **Assign #N**. With the web password the claim above then numbers it within seconds; without it, it waits |
| Register page Number step | With web numbering, `regflow.step_number` asks the web (`hub.request_number`) and waits: "Getting a new number from the web…", or on failure "The web could not hand out a number (‹error›); retrying. Or assign one by hand." Without a web password it calls `suggested_number()` (lowest free above 32, local). Either way a new number shows **NEW NUMBER: write #N on the cube's label**. Only the local path can collide with another computer; the sync merge then keeps one claim and drops the other device to **Needs number** ({{page:X08}}) |
| **Pair new cubes (auto)** | `controller.choose()` skips a discovered row whose number is NULL. With web numbering the new cube gets its number from the web within seconds and is then paired; without the web password it waits in **Needs number**: assign the number first (card, or **Rename**), or use the Register page |
...
| **Signal the cube when its tag is read** selector (`radio.reader_flash {on?, action?}`): **Off** · **Flash 2 s** · **idle** · **preshow** · **desert** · **pool** · **mainshow** | Workstation panel › **On the reader** card (Workstation firmware, `nfc` role, only; hidden on the legacy pairing station) | ON by default (setting `reader_flash`, saved in `console_settings`, one setting for every Workstation; also Settings › Behaviour). Runs in the hub whether or not the page is open. Each tag placed makes its owning cube (committed `uid`, else `pending_uid`) either do a 2 s identify flash (blue/red, then idle white; default action **Flash 2 s**) or receive one `SET_ZONE N` (**idle** 0 · **preshow** 1 · **desert** 2 · **pool** 3 · **mainshow** 4; the zone stays). Action saved in metadata `console_reader_action` (`flash` \| `zone:0`…`zone:4`). Sends nothing while this Controller is busy, another link's Controller is working with that cube, or a main show the console knows about is running; the flash is a background flash (Controller mode `reader_flash`) that any operation takes over at once (a `stop` goes first). One action per placement (lift and place again; a re-handshake does not repeat it). A setting, not an on-demand action: the card has no one-shot flash or zone buttons (manual SET_ZONE: **Cubes & show**). The legacy pairing station never acts (manual flash from the Pairing tab or cube card only). No cube firmware change, no NVS write, no database write, not an `nfc_seen` scan. Results in "Last automatic action": flashed / flashed (pending tag) / zone N sent / nothing sent: unknown tag / nothing sent: the radio was busy / nothing sent: excluded device / nothing sent: a main show is running. Simulation-verified. Procedure: {{page:H3}} B3; detail {{page:X11}} |
...
| Another cube plugged in mid-registration | `hub.identified()` stops the running controller operation before pinning, and logs `Stopped the active pairing operation before pinning <MAC> (identified on <port>)`. A tag-read background flash (mode `reader_flash`) is not stopped here and nothing is logged: it finishes its 2 s, or the next operation started takes it over | Check the pinned cube; start again deliberately. An interrupted registration never continues on another cube |
...
> [!WARNING] **Two kinds of "flash".** **Identify (flash until Stop)**, **Flash 2 s**, **Flash LEDs until Stop** (cube card) and the Workstation tag-read action **Signal the cube when its tag is read** (on by default; its **Flash 2 s** action) only blink the LEDs; its zone actions send one `SET_ZONE`. None writes firmware or NVS. **Flash cube firmware** (cube **Firmware** tab) and the **Flash cubes** page write firmware.
...
- Condition: the cube's verified `?` answer reports an `FW:` **older** than `core.VERSION` (v1.7.0-USB.1); only upgrades, never downgrades (`autoupgrade.compare_versions`, working tree 24 Sept, Simulation-verified). Numbers compare numerically and the `-USB.n` suffix counts (`v1.7.0-USB.2` is newer than `v1.7.0-USB.1`; plain `v1.7.0` is older). A newer cube is listed (**by hand**) with "Newer than this computer's build (‹version›); not downgraded"; a cube whose version cannot be ordered (for example a different suffix label, `-RC.1`) is listed with "Cannot tell whether ‹running› is older than ‹version›; not flashed automatically (flash it by hand if it should change)". An unverified cube (no matching `Cube MAC:` / `FW:` / READY) and a MAC whose role is `excluded` are left alone. A cube whose FW matches is not listed.
...
- Cubes heard over the air with older (or unorderable) firmware (reported through the show registry in the last day) are counted in the panel line "{n} heard over the air in the last day with older firmware: plug in over USB to upgrade". Firmware never changes over the air.
