...
| Sync switches local automatic numbering off | Every web sync (`pairing_station/web_sync.py`, "Independent computers cannot safely allocate the next free number") and every Git inventory apply (`inventory_sync.py`) writes `auto_number=0`; nothing sets it back to 1. The live database holds `0` |
