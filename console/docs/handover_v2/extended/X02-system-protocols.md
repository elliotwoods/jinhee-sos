...
Role changes: the cube panel's **Role** selector (`inventory.set_role`; **auto (cube)**, **LED · manually assigned**, **Reader / base station (excluded)**) moves a board into or out of cube service. Refused while a station operation runs: "Finish or stop the current station operation before changing a role" (selector tooltip "Stop the active operation first"); a tag-read background flash (mode `reader_flash`) is ended instead of refusing.
...
| Tag-read action (console) | On by default (`reader_flash`, one setting for every Workstation): each tag placed on a board with the `nfc` role makes its owning cube do a 2 s identify flash, or one `SET_ZONE N` if chosen (`console_reader_action`). Only when that radio is idle, no other link works with the cube and no main show is known to run; any operator action takes over (a `stop` first). A legacy pairing station never does it. No firmware change on either side. Detail {{page:X11}}; Simulation-verified |
