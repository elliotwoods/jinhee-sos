...
There are two ways to register. **The Register page (⌘3) is the normal one:** switch it on, then plug the cubes in one after another. It numbers each new cube, asks for its tag and syncs. **The cube card (manual path)** does the same steps one click at a time; use it for a single re-registration, a cube that cannot be plugged in over USB, or when the Register page is waiting on something you need to fix by hand.
등록 방법은 두 가지입니다. **Register 페이지(⌘3)가 기본 방법입니다:** 켜 두고 큐브를 하나씩 꽂으면 새 큐브 번호 부여, 태그 스캔 요청, 동기화까지 진행합니다. **큐브 카드(수동 경로)**는 같은 단계를 한 번에 하나씩 클릭으로 진행합니다. 한 대만 재등록할 때, USB로 연결할 수 없는 큐브, 또는 Register 페이지가 수동으로 해결해야 할 문제로 대기 중일 때 사용합니다.
...
1. **Open Register** (top bar, after Flash, ⌘3). The chip under the title must say **Station ready · NFC ok**; plug the pairing station in first. Switch on the bar **Register cubes as they are plugged in** at the top of the page. The switch is saved on this computer and is off by default. A cube that is already plugged in starts straight away.<br>상단 Flash 다음 항목 **Register**(⌘3)를 엽니다. 제목 아래 칩이 **Station ready · NFC ok**여야 하며 먼저 등록 스테이션을 꽂습니다. 페이지 상단의 **Register cubes as they are plugged in** 바를 켭니다. 이 설정은 이 컴퓨터에 저장되며 기본값은 꺼짐입니다. 이미 꽂혀 있는 큐브는 바로 시작됩니다.
2. **Plug in a cube over USB.** The steps run in order: **USB** (identified and pinned) → **Number** → **NFC scan** → **Sync**. If the **Flash cubes** page is also on (chapter 04), the cube is flashed first and this page shows "Waiting for the Flash page to finish this cube".<br>큐브를 USB로 꽂습니다. **USB**(식별·고정) → **Number** → **NFC scan** → **Sync** 순으로 진행됩니다. **Flash cubes** 페이지도 켜져 있으면(04장) 먼저 플래시하며, 이 페이지에 "Waiting for the Flash page to finish this cube"가 표시됩니다.
