...
ACTIONS = {
    ...
    'radio.show_start_all': dict(label='준비된 ALL 큐브에서 쇼 시작', what='범위 내 메인쇼 준비 상태인 모든 큐브에 MSG_SHOW_START 브로드캐스트.',
                                 hazard='브로드캐스트이며 확인 응답이 없고 되돌릴 수 없습니다. 길게 눌러 확인하세요.'),
    'radio.reader_flash': dict(label='태그를 읽으면 큐브에 신호', what='기본으로 켜짐: Workstation 리더에 태그를 올릴 때마다 그 태그의 주인 큐브가 2초간 파랑/빨강으로 점멸(또는 선택한 경우 존 설정)하며, 페이지가 열려 있지 않아도 동작합니다. 태그·인벤토리·무선이 모두 일치하는지 볼 수 있습니다.',
                               hazard='큐브에서 보입니다: 점멸은 대기 흰색으로 끝나고, 존은 설정된 채로 남습니다. 무선이 작업 중이거나 메인쇼가 진행 중이면 아무것도 보내지 않으며, 직접 시작한 작업이 즉시 우선합니다.',
                               needs='NFC 리더가 동작하는 Workstation (기존 등록 스테이션 제외).'),
