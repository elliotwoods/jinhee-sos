...
export function SettingsSection() {
  ...
  return html`<div><${PageHead} title=${t('Settings')} /><${Explainer} id="settings" />
    ...
    <div class="card" data-doc="settings.auto"><h3>${t('Automatic updates')}</h3><div class="stack">
      ...
      <label class="check"><input type="checkbox" checked=${!!s.auto_build} onChange=${toggle('auto_build')} /> ${t('Build firmware when its source changes (cube, zone plates, Workstation, Mainshow controller), one build at a time, before anything is flashed from it')}</label>
