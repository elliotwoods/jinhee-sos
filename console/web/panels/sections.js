...
export function SettingsSection() {
  ...
  return html`<div><${PageHead} title=${t('Settings')} /><${Explainer} id="settings" />
    ...
    <div class="card" data-doc="settings.auto"><h3>${t('Automatic updates')}</h3><div class="stack">
      <label class="check"><input type="checkbox" checked=${!!s.auto_zone_db_radio} onChange=${toggle('auto_zone_db_radio')} /> ${t('Zone databases over the air: every zone relay walks the zone database to the out-of-date zones its radio hears, one radio at a time')}</label>
      ...
      <label class="check"><input type="checkbox" checked=${!!s.auto_sync} onChange=${toggle('auto_sync')} /> ${t('Sync the inventory with the web by itself: local changes upload a few seconds later, web changes download, new cube mappings publish (needs the web password)')}</label>
      ${meta.packaged ? null : html`<label class="check"><input type="checkbox" checked=${!!s.auto_build} onChange=${toggle('auto_build')} /> ${t('Build firmware when its source changes (cube, zone plates, Workstation, Mainshow controller), one build at a time, before anything is flashed from it')}</label>`}
      <label class="check"><input type="checkbox" checked=${!!s.auto_firmware_usb} onChange=${toggle('auto_firmware_usb')} /> ${t('Upgrade the firmware of USB boards that are out of date: zone plates keep their identity, a pairing station or General Radio becomes a Workstation, cubes only while Register and Flash are off. Never while you are using the board; pool radios, the pool central and the preshow bridge are only listed')}</label>
      <div class="note">${t('Saved on this computer; on by default.')}</div></div></div>
    <div class="card"><h3>${t('Keyboard')}</h3><div class="stack note"><div><kbd>Esc</kbd> ${t('close a tooltip, else stop every active operation')}</div><div><kbd>/</kbd> ${t('search')}</div><div><kbd>⌘1</kbd>–<kbd>⌘7</kbd> ${t('sections')}</div><div><kbd>j</kbd>/<kbd>k</kbd> ${t('move the rail selection')}</div><div><kbd>?</kbd> ${t('this list')}</div></div></div>
