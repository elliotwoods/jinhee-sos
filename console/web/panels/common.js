...
export function DeviceHeader({ device, title, pills, kv }) {
  ...
  const meta = html`<${Pill} status=${'usb.' + device.state} /> ${pills}
    <span class="chip mono" title=${t('port')}>${device.port}</span>
    ${device.mac && html`<span class="chip mono" title="MAC"><${Mac} mac=${device.mac} /></span>`}
    ${device.firmware && html`<span class="chip mono" title=${t('firmware')}>${device.firmware}</span>`}`;
  const actions = html`
    ${device.state !== 'job' && html`<${ActionButton} name="device.probe" args=${{ device: device.id }} label=${t('Identify again')} className="btn small quiet" title=${t('Ask the board what it is (no reset)')} />`}
    ${device.pinned && html`<${ActionButton} name="device.unpin" args=${{}} label=${t('Unpin')} className="btn small" />`}
    ${device.state === 'session' && html`<${ActionButton} name="device.disconnect" args=${{ device: device.id }} label=${t('Disconnect')} className="btn small" />`}
    ${canConnect && html`<${ActionButton} name="device.connect" args=${{ device: device.id }} label=${t('Connect')} className="btn small primary" />`}`;
  ...
  return html`<${PageHead} title=${title || deviceLabel(device)} meta=${meta} actions=${actions} />
    ${facts && html`<div class="card">
      ${device.presumed && device.presumed.label && html`<div class="note">${t('Inventory:')} ${withMacs(device.presumed.label)}</div>`}
...
export function RawConsole({ device, hint }) {
  return html`<div class="card"><h3>${t('Console')}</h3><${Console} device=${device.id} hint=${hint || '?'} /></div>`;
}
...
export function JobHistory({ device }) {
  ...
  return html`<div class="card"><h3>${t('Recent jobs on this port')}</h3><div>${jobs.map((j) => html`<${JobCard} key=${j.id} job=${j} />`)}</div></div>`;
}
