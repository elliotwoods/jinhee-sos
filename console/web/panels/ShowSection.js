...
import { rowByNumber } from './common.js';
...
export function ShowControl() {
  ...
  return html`<div class="card"><${Explainer} id="show" />
    ...
    <h3>Show clock</h3>
    ${s ? html`<div class="stack" data-doc="show.clock"><div class="row"><span class="readout">${mmss(s.elapsed_s)}</span><span class="note">${s.segment}</span></div><${ProgressBar} value=${100 * s.elapsed_s * 1000 / (s.length_ms || 1)} /><div class="note">Show ${s.show_id} · ${s.name} · started ${s.source}</div></div>` : html`<div class="note" data-doc="show.clock">No show started from this controller yet.</div>`}
