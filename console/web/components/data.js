...
export function DataTable({ columns, rows, keyOf, selected, onSelect, toneOf, empty = t('Nothing to show'), maxRows = 400, doc, rowDoc }) {
  ...
  return html`<div class="table-wrap" data-doc=${doc}><table class="data"><thead><tr>${columns.map((c) => html`<th onClick=${() => setSort({ key: c.key, dir: sort.key === c.key ? -sort.dir : 1 })} scope="col">${c.label}${sort.key === c.key ? (sort.dir > 0 ? ' ▲' : ' ▼') : ''}</th>`)}</tr></thead>
    ...
    ${sorted.length > maxRows && html`<tfoot><tr><td colspan=${columns.length} class="note">${t('{n} more rows (narrow the filter)', { n: sorted.length - maxRows })}</td></tr></tfoot>`}</table></div>`;
}
...
export function LogPane({ device, kind = 'line', filter, height = 220, items }) {
  ...
  return html`<div class="log-wrap"><div class="log" ref=${box} style=${`height:${height}px`} onScroll=${onScroll}>
    ...
    ${!rows.length && html`<div class="muted">${t('Nothing yet')}</div>`}</div>
    ${paused && pending > 0 && html`<button class="btn small log-new" onClick=${() => { box.current.scrollTop = box.current.scrollHeight; setPaused(false); setPending(0); }}>${t('{n} new lines ↓', { n: pending })}</button>`}</div>`;
}
...
export function Console({ device, hint = 'help', height = 240, filter }) {
  ...
  return html`<div class="console"><div class="row"><input class="field filter" placeholder=${t('Filter lines')} value=${q} onInput=${(e) => setQ(e.target.value)} /></div>
    <${LogPane} device=${device} height=${height} filter=${f} />
    <form class="console-input" onSubmit=${send}><input value=${text} onInput=${(e) => setText(e.target.value)} onKeyDown=${key} placeholder=${t('Send a line (e.g. {example})', { example: hint })} spellcheck="false" autocomplete="off" /><button class="btn" data-doc="device.console">${t('Send')}</button></form></div>`;
}
...
export function JobCard({ job, compact }) {
  ...
  return html`<div class=${'job ' + (job.state === 'running' ? 'running' : tone)} data-doc=${`job:${job.kind}`}><div class="head"><strong>${job.title}</strong><${Pill} tone=${tone} label=${job.state} /></div>
    ...
    ${!compact && job.log_tail && job.log_tail.length ? html`<details><summary class="note">${t('Log ({n})', { n: job.log_tail.length })}</summary><div class="log short">${job.log_tail.map((l) => html`<div>${l}</div>`)}</div></details>` : null}
    ${job.state === 'running' && job.cancellable && html`<${ActionButton} name="jobs.cancel" args=${{ job: job.id }} label=${t('Cancel')} className="btn small" />`}</div>`;
}

export function PortTable({ devices, onSelect, selected }) {
  const columns = [
    { key: 'port', label: t('Port'), mono: true }, { key: 'role_label', label: t('Identified as') },
    { key: 'mac', label: 'MAC', mono: true, render: (d) => d.mac || d.usb_mac || '—' },
    { key: 'firmware', label: t('Firmware'), mono: true },
    { key: 'state', label: t('State'), render: (d) => html`<${Pill} status=${'usb.' + d.state} />` },
    { key: 'presumed', label: t('Inventory says'), render: (d) => (d.presumed && d.presumed.label) || '—' },
    { key: 'error', label: t('Note'), render: (d) => d.error || '' },
  ];
  return html`<${DataTable} columns=${columns} rows=${devices} keyOf=${(d) => d.id} selected=${selected} onSelect=${onSelect} empty=${t('No USB serial devices')} />`;
}
