...
Power off other Poolzone radio transmitters during testing. The bridge emulates **all six radio IDs**, including release packets for unselected slots, so real radios using those IDs would compete with it. Brightness, colour and all-23-on are not supported by the existing central protocol. PCA9685 output indices 1–16 are 0x40 channels 0–15 and 17–24 are 0x41 channels 0–7 (one output has no frame), but the outputs are **not** wired
