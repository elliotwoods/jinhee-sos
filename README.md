...
If the same device changed on this computer and on the web, sync resolves it automatically when only its status, detail or timestamp differ (the newest record wins, e.g. the latest acknowledgment), or when only one side changed its number, tag or role (that side wins). Only when both changed the number, tag or role differently does the app list a conflict and change nothing until you choose **keep local** or **take web**. The pairing, cube-flasher, zone-flasher and calibration apps show a one-line web inventory status (up to date / newer web changes / local changes not uploaded / offline). They never block or fail when the web is unreachable. Headless equivalent: `pairing_station/.venv/bin/python scripts/web_sync.py status|sync`.
