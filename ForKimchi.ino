...
=========================================================
 ...
 - Router connection and OTA removed; ESP-NOW uses fixed channel 2
...
#include <WiFi.h>  // Required radio driver for ESP-NOW; no router connection.
#include <esp_now.h>
...
// =====================================================
// ESP-NOW radio channel (must match all controllers/readers)
// =====================================================
...
enum MessageType : uint8_t {
  ...
  // Value 5 is reserved; keep all existing protocol IDs unchanged.
...
void setup() {
  ...
  // ===================================================
  // ESP-NOW requires the Wi-Fi radio in station mode, without joining an AP.
  // Disable reconnects so saved router credentials cannot move the channel.
  // ===================================================
  ...
  if (!WiFi.mode(WIFI_STA) || !WiFi.disconnect()) {
    Serial.println("ESP-NOW RADIO INIT ERROR");
    showError();
    return;
  }
  ...
  if (
    ...
    Serial.println("ESP-NOW RADIO CHANNEL/POWER CONFIG ERROR");
    showError();
    return;
  }
  ...
  if (esp_now_register_recv_cb(onDataRecv) != ESP_OK) {
    Serial.println("ESP-NOW RECEIVE CALLBACK ERROR");
    showError();
    return;
  }
