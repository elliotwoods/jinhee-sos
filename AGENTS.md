...
| Workstation | `zones/firmware/Workstation/`, `zones/tools/workstation.py` (`general_radio.py` remains as an alias), `zones/dbmanager/dongle.py` | One ESP32-C3 for every ESP-NOW host function, the merge of the pairing-station and General Radio firmwares (`workstation-1.0.0`): the General Radio 1.2.0 protocol (cube discover/identify/register, zone frame relay, the Mainshow verbs `set_zone` to one cube or spelled-out `broadcast` and `show_start`, the main-show relay `show_send`/`show_frame` and timecode used by the console Show editor, one emulated pool radio, an emulated preshow plate, `led_test`) plus the pairing station's PN532 reader (SDA GPIO4 / SCL GPIO3; polls only while a host sends `nfc_poll enabled:true`). Leased outputs. Hosts recognise a board by its hello capability fields (`dongle.py`: `has_reader`, `relay_capable`, `show_capable`…), not its firmware name, so legacy nct-pairing-1.8-zones stations and general-radio-1.0.0–1.2.0 boards (`GeneralRadio` sketch, now replaced and no longer a flash target) keep working. Protocol: its `README.md`. Python client + bench CLI + `flash`. Not a zone board, not the Mainshow controller |
...
| Show editor | `console/web/panels/ShowEditor.js`, `console/showedit.py`, `pairing_station/show_registry.py`, `show_publish.py`, `web/src/lib/show.ts` | Edit the main show as cues with a cube-exact preview, publish web-allocated show versions, update cubes over a Workstation or general-radio-1.1.0+ (update all / auto walk-around), send the show length to the controller |
| Zone Database Manager | `zones/dbmanager/app.py` | Wireless zone discovery/version view, targeted, update-all and auto-update-all database updates over an ESP-NOW dongle (a Workstation, or a legacy pairing station/General Radio), dongle flashing (writes the Workstation firmware), web publish/pull |
...
- Five protocols share channel 2 and are **not** interchangeable: the cube `Packet`
  ...
  show editor's broadcast (every ~60 ms, about 16 Hz; 600 ms lease) of per-number colours that a registered, non-playing v1.7.0+ cube shows
  for a lease and then restores, relayed broadcast-only by general-radio-1.2.0 and the Workstation). The preshow media bridge additionally claims
...
**Documentation has one owner.** A single designated documentation session (named `handover-docs-split` on
2026-09-23; find it with ListAgents) writes all documentation: READMEs, this file, `docs/`, `console/README.md`,
TEST_REPORT, `console/docs/` (handbook `handover_v2/handbook/`, extended reference `handover_v2/extended/`, PDF via
`console/tools/handover_pdf/`), and the Notion handover. Other agents do not edit documentation. When a change needs
docs, send that session the facts: behaviour, defaults, exact UI strings, source file/commit, and evidence level
(simulation / bench / field). Console UI strings and their Korean stay with their code owners.
