"""General radio: the Python side of zones/firmware/GeneralRadio.

One ESP32-C3 dongle that performs every ESP-NOW host function: the pairing-station relay
(discover / identify / register cubes, relay zone-management frames), the Mainshow controller's
verbs (set a cube's zone, start the main show), one emulated pool slider radio (hold a pool lamp
through the pool central) and an emulated preshow plate (a TouchDesigner cue through the media
bridge). The dongle speaks one JSON object per line at 115200; every request carries an `id`
and its reply echoes it. See zones/firmware/GeneralRadio/README.md for the protocol.

`GeneralRadio` is a synchronous client over pairing_station/transport.py (its serial worker, the
...
board through their own sessions. The dongle leases its pool and preshow outputs on the host's
...
Run as a script for bench checks and for putting the firmware on a spare board:

    pairing_station/.venv/bin/python zones/tools/general_radio.py --port <port> hello
...
class GeneralRadio:
    ...
    @property
    def general(self):
        return is_general(self.firmware)
    ...
    def status(self):
        return self.request('status') if self.general else self.hello()
    ...
    def _needs(self, what, ok):
        if not ok:
            raise RadioError(f'{what} needs the general radio firmware ({dongle.GENERAL.version}); '
    ...
    def set_zone(self, target, zone):
        ...
        self._needs('set_zone', self.general or self.firmware.startswith('mainshow-'))
    ...
    def show_start(self, target='broadcast'):
        self._needs('show_start', self.general or self.firmware.startswith('mainshow-'))
        return self.request('show_start', target=target)

    # ---- the general radio's own roles ----
    ...
    def preshow(self, point, state):
        ...
        self._needs('preshow', self.general)
...
def flash_general(path, database, emit=print, backup='always'):
    """Put the general radio firmware on the spare ESP32-C3 at `path` (never a cube, zone or the station).
    ...
    try:
        ...
        mac = dongle.flash(port, known, folder, lambda kind, value: emit(f'{kind}: {value}'), firmware=dongle.GENERAL, backup=backup)
...
def parser():
    p = argparse.ArgumentParser(description='Drive a general radio dongle (zones/firmware/GeneralRadio).')
    ...
    f = sub.add_parser('flash', help='write the general radio firmware to the spare board on --port')
...
def main(argv=None, radio=None, db=None, out=print):
    ...
    try:
        if args.command == 'flash':
            mac = flash_general(args.port, args.database, emit=out, backup=args.backup)
            out(json.dumps(dict(flashed=mac, firmware=dongle.GENERAL.version, role='excluded')))
            return 0
        own_radio, own_db = radio is None, db is None
        radio = radio or GeneralRadio(on_event=lambda e: out(json.dumps(e)))
