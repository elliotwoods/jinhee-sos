...
- **Controller**:
  ...
  - **Flash controller firmware…** converts a spare ESP32-C3 dongle, such as an ex-cube board running the Workstation firmware or a legacy pairing-station relay. The Zone Database Manager then needs the other dongle. A port that app has open is refused.
  - A **Workstation** (`zones/firmware/Workstation`) or a legacy General Radio (general-radio-1.x) also answers ① and ② (with the plate-style colour repeats), but it has no BOOT-button/trigger-input show start and is never recorded as the controller (the status says "no physical trigger (workstation)").
