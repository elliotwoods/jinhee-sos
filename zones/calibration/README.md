...
**Update firmware** performs the identity check again before uploading, reuses a checksum-verified current build (compiles only if missing/stale), disarms the Python override, backs up the complete 4 MB flash, validates the existing partition layout, and writes only the application. It verifies the program plus unchanged NVS calibration and zone/database partitions, confirms the rebooted version/fingerprint, then reconnects with the override disarmed. The tab shows progress and tool output; backups and logs are retained under `build/firmware-runs/`. Matching builds are skipped.
