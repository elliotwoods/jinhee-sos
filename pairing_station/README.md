...
- **Zones…** replaces *Export reader table* for zone boards built on NctZone (currently PreshowZone). **Publish database** broadcasts the committed mappings over ESP-NOW to every zone in range. The table shows each zone's database version, update progress and health. This needs station firmware `nct-pairing-1.6-zones`. See `zones/README.md`.
