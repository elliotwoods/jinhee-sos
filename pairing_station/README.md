...
- **Zones → Zone Database Manager…** opens the separate manager (`zones/dbmanager/`), which updates zones over its own ESP-NOW dongle. The station itself can serve as the dongle while this app is disconnected from it, and so does a Workstation (`zones/firmware/Workstation`, a superset of the station protocol including the reader) or a legacy General Radio (general-radio-1.x, the same protocol without the reader). Station firmware `nct-pairing-1.7-zones` adds signal strength; `1.8` adds zone RX gain control. The station firmware in `firmware/pairing_station` is frozen at `nct-pairing-1.8-zones`, which the installed station runs; new boards get the Workstation firmware. See `zones/README.md`.
